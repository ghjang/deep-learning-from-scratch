...
* __파이썬 슬라이싱과 NumPy 배열 슬라이싱의 관계__

  파이썬의 기본 슬라이싱은 일차원 시퀀스를 위한 것이지만, NumPy는 이 개념을 다차원 배열로 확장했다:
  ...
  print(my_list[2:7:2])  # [2, 4, 6] - 일차원 슬라이싱
  ...
  # 콤마로 구분된 여러 슬라이스 - 각 차원별로 슬라이싱 적용
  print(array_2d[0:2, 1:3])
  # 출력:
  # [[1 2]
  #  [5 6]]
  
  # 첫 번째 차원에서 모든 요소, 두 번째 차원에서 인덱스 1 요소
  print(array_2d[:, 1])  # [1 5 9]
  ```

  NumPy 슬라이싱의 주요 특징:
  
  1. __차원별 슬라이싱__: 각 차원마다 별도의 슬라이싱을 콤마로 구분해 지정할 수 있다
  2. __뷰(View) 반환__: 대부분의 경우 데이터 복사 없이 원본 배열의 뷰를 반환한다
  3. __고급 인덱싱__: 불리언 마스크, 정수 배열 등을 사용한 고급 인덱싱 제공

  기본적인 슬라이싱 문법(`[start:stop:step]`)은 파이썬 시퀀스와 NumPy 배열 모두에서 동일하지만, NumPy는 다차원 데이터 처리를 위해 이 개념을 확장했다고 볼 수 있다. NumPy의 다차원 배열 슬라이싱에 대한 자세한 내용은 [2.4 NumPy 인트로](../2_modules_related_to_ds/2_4_numpy_intro.md#244-배열-인덱싱과-슬라이싱) 섹션을 참조하라.
