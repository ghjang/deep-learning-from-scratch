...
for epoch in range(epochs):
    ...
    # 100 에폭마다 가중치 저장 (시각화를 위해)
    if epoch % 100 == 0:
        # 현재 가중치와 편향을 깊은 복사하여 저장
        weights = {
            'W1': W1.copy(),
            'b1': b1.copy(),
            'W2': W2.copy(),
...
# 가중치 히스토리 저장
with open('xor_weight_history.pkl', 'wb') as f:
    pickle.dump(weight_history, f)
    
print(f"학습 과정 가중치가 'xor_weight_history.pkl'에 저장되었습니다. (총 {len(weight_history)}개 시점)")
